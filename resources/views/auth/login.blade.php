    ...
    <div class="flex-grow flex items-center justify-center">
        ...
        >
            <div class="bg-white p-6 sm:p-8 rounded-xl border border-gray-100 border-t-4 border-navy-primary shadow-xl shadow-slate-200/50">
                <x-auth-session-status class="mb-5 rounded-lg border border-teal-primary/30 bg-teal-primary/10 px-4 py-3 text-sm text-teal-primary" :status="session('status')" />

                <h1 class="text-2xl font-bold text-center text-dark-slate">Customer Login</h1>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-5" @submit="isSubmitting = true">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input
                                ...
                                :class="(touchedPassword && password.length === 0) || hasPasswordError ? 'border-coral-accent focus:ring-coral-accent focus:border-coral-accent' : ''"
                            />
                        ...
                        <p x-cloak x-show="touchedPassword && password.length === 0" class="mt-1.5 text-xs text-coral-accent">
                    ...
                    <div class="flex items-center justify-between text-sm mb-6">
                        <label for="remember_me" class="inline-flex items-center gap-2 text-gray-600">
                            <input id="remember_me" type="checkbox" name="remember" class="w-4 h-4 text-teal-primary border-gray-300 rounded focus:ring-teal-primary">
                            <span>{{ __('Remember me') }}</span>
                        </label>

                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-navy-primary font-medium hover:underline hover:text-[#083a6a]">
                                {{ __('Forgot Password?') }}
                            </a>
                        @endif
                    </div>
                ...
                @if (Route::has('register'))
                    <p class="mt-6 text-center text-sm text-gray-600">
                        New here?
                        <a href="{{ route('register') }}" class="text-navy-primary font-medium hover:underline hover:text-[#083a6a]">Create an account</a>
                    </p>
                @endif
            </div>
        </div>
